		...
		foreach ($modulesdir as $dir)
		{
            ...
            foreach($modulenames as $modulename)
            {
				...
				if ($qualified)
				{
					...
					if ($allowaddtarget)
					{
						print '<form name="'.$modulename.'" action="'.$_SERVER['PHP_SELF'].'?action=add&token='.newToken().'&id='.$object->id.'&module='.$modulename.'" method="POST" enctype="multipart/form-data">';
			...
			while ($i < min($num,$conf->liste_limit))
			{
				...
				if ($obj->statut == 0)
				{
					...
					if ($user->rights->mailing->creer && $allowaddtarget) {
						print '<a href="'.$_SERVER['PHP_SELF'].'?action=delete&token='.newToken().'&rowid='.$obj->rowid.$param.'">'.img_delete($langs->trans("RemoveRecipient"));
					}
					print '</td>';
				}
				else
